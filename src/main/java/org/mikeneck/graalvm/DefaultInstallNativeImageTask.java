...
public class DefaultInstallNativeImageTask extends DefaultTask implements InstallNativeImageTask {
  ...
  public DefaultInstallNativeImageTask(Provider<GraalVmHome> graalVmHome) {
    ...
    getOutputs().upToDateWhen(task -> nativeImageCommand().filter(Files::exists).isPresent());
  }
  ...
  public void installNativeImage() {
    ...
    Path graalVmUpdaterCommand =
        graalVmUpdaterCommand()
            .orElseThrow(
                () ->
                    new InvalidUserDataException(
                        String.format(
                            "graalVM updater command does not exist in %s.", graalVmHome)));
