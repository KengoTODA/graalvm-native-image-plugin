...
public class Case60ProxyUsageArrayTest {
  ...
  void runCase() {
    ...
    assertAll(
        ...
        () -> assertThat(context.file("build/tmp/native-image-config/out-0")).exists(),
        () -> assertThat(context.file("build/tmp/native-image-config/out-1")).exists(),
        () -> assertThat(context.file("build/tmp/native-image-config/out-2")).exists(),
        () ->
            assertThat(context.fileTextContentsOf("build/native-image-config/proxy-config.json"))
  ...
  void createNativeImage() {
    FunctionalTestContext context =
        new FunctionalTestContext("case60-proxy-usage-array", "case60-native-image");
    context.setup();
